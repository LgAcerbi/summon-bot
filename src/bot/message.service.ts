...
export class MessageService {
  getMessage(userData: any, championsData: any): any {
    ...
    const championData = this.getBiggestMasteryChampion(
      userData.masteries,
      championsData,
    );
  ...
  formatEmbedMessage(userData: any, championData): EmbedMessageDto {
    const embedMessage = new MessageEmbed()
      ...
      .setThumbnail(
        `http://ddragon.leagueoflegends.com/cdn/11.7.1/img/profileicon/${userData.profileIconId}.png`,
      )
      .addFields(
        {
          name: 'Level',
          value: `:small_blue_diamond: ${userData.summonerLevel}`,
          inline: true,
        },
        {
          name: 'Mastery',
          value: `:small_orange_diamond: ${userData.score} points`,
          inline: true,
        },
        {
          name: '\u200B',
        ...
        {
          name: 'Solo/Duo',
          value: this.getEloField(userData.elo.solo),
          inline: true,
        },
        {
          name: 'Flex',
          value: this.getEloField(userData.elo.flex),
          inline: true,
        },
        {
          name: '\u200B',
      ...
      .addField(
        'Biggest Mastery',
        `Level: ${userData.masteries[0].championLevel}  -  Points: ${userData.masteries[0].championPoints}`,
        false,
      )
      .setImage(
        `http://ddragon.leagueoflegends.com/cdn/11.7.1/img/champion/${this.parseChampionName(
          championData.name,
        )}.png`,
      );
  ...
  formatEloData(eloData) {
    ...
    formattedElo.solo.winrate = this.getWinrate(
      formattedElo.solo.wins,
      formattedElo.solo.losses,
    );
    formattedElo.flex.winrate = this.getWinrate(
      formattedElo.flex.wins,
      formattedElo.flex.losses,
    );
    return formattedElo;
  }

  getBiggestMasteryChampion(
    masteriesData: FindMasteriesBySummonerIdDto[],
    champions: any,
  ) {
    champions = Object.values(champions.data);
    return champions.find(
      (champion) => champion.key === masteriesData[0].championId.toString(),
    );
  }
